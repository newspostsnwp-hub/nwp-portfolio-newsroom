...
Default to hands-off: let the News Feed Team (news-sourcing-agent,
news-scoring-agent, news-ops-agent, news-frontend-agent) run reviews and
report back rather than seeking approval at every step for in-scope changes.
At the same time, stay token-conscious — don't spend background effort on
review cycles beyond what's actually asked for.
...
others does nothing real (discovered the hard way: it silently faked a
delegation instead of actually calling a tool). Orchestration (deciding which
specialist(s) apply, invoking them, aggregating their reports) is handled
directly by the main assistant - either via the `/newsfeed-review` skill or
when the user asks in plain language (e.g. "review the news feed"). Don't
recreate a coordinator subagent for this.

news-sourcing-agent and news-scoring-agent both edit scripts/update_news.py
(different sections) - they must run sequentially, never in the same
turn/batch, to avoid conflicts on that shared file. news-ops-agent is
diagnostic-only for the Cloudflare scheduler worker and refresh-news.yml - it
never deploys and never commits.
